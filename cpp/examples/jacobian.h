...
class DifferentPnPJacobianAccumulator {
   public:
    ...
    DifferentPnPJacobianAccumulator(const ConstRefRowMajorMatrixX2f& points2D_src,
                                    const ConstRefRowMajorMatrixX2f& points2D_tgt, const CameraState& camera_tgt,
                                    const std::vector<Plane>& planes, bool optimize_focal_length,
                                    bool optimize_principal_point, LossFunction loss_fn, ResidualWeightVector weights)
    ...
    float residual(const CameraState& camera_src) const {
        float cost = 0;
        for (Eigen::Index i = 0; i < points2D_src.rows(); ++i) {
            ...
            // FIXME
            const Eigen::Vector3f point_world = *Intersect(ray, plane);
    ...
    size_t accumulate(const CameraState& camera_src, RowMajorMatrixf<9, 9>& JtJ, RowMajorMatrixf<9, 1>& Jtr) const {
        ...
        for (Eigen::Index i = 0; i < points2D_src.rows(); i++) {
            ...
            CameraPose::ApplyWithJac(X, camera_tgt_R, camera_tgt_t, &XCam, &dXCam_dX);
    ...
    CameraState step(RowMajorMatrixf<9, 1> dp, const CameraState& camera_src) const {
        CameraState camera_new = camera_src;
        camera_new.pose.q = quat_step_post(camera_src.pose.q, dp.block<3, 1>(0, 0));
        camera_new.pose.t = camera_src.pose.t + dp.block<3, 1>(3, 0);

        if (optimize_focal_length) {
            camera_new.intrinsics.fy = camera_src.intrinsics.fy + dp(6, 0);
            camera_new.intrinsics.fx = camera_new.intrinsics.fy * camera_new.intrinsics.aspect_ratio;
        }
        if (optimize_principal_point) {
            camera_new.intrinsics.cx = camera_src.intrinsics.cx + dp(7, 0);
            camera_new.intrinsics.cy = camera_src.intrinsics.cy + dp(8, 0);
        }
