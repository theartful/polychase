...
static inline void SolvePnPIterative(const RefConstRowMajorMatrixX3f &object_points,
                                     ...
                                     const LossFunction &loss_fn, const PnPOptions &opts, PnPResult &result) {
    PnPProblem problem(image_points, object_points, weights, opts.optimize_focal_length, opts.optimize_principal_point,
    ...
    result.bundle_stats = LevMarqDenseSolve(problem, loss_fn, &params, opts.bundle_opts);
    ...
    if (opts.max_inlier_error > 0.0f) {
        const Float max_inlier_error_sq = opts.max_inlier_error * opts.max_inlier_error;
    ...
    result.inlier_ratio = static_cast<Float>(num_inliers) / problem.NumResiduals();
}
...
#define SWITCH_LOSS_FUNCTION_CASE(LossFunction)                                         \
    {                                                                                   \
        ...
        SolvePnPIterative(object_points, image_points, weights, loss_fn, opts, result); \
    }
        SWITCH_LOSS_FUNCTIONS
#undef SWITCH_LOSS_FUNCTION_CASE
        default:
            throw std::runtime_error(
                fmt::format("Unknown loss type: {}", static_cast<int>(opts.bundle_opts.loss_type)));
    }
}
