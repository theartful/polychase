...
class OpenGLPnPAdapter {
   public:
    OpenGLPnPAdapter(const RefConstRowMajorMatrixX3f& object_points,
                     PnPResult& result)
        : object_points_(const_cast<float*>(object_points.data()),
                         object_points.rows(), object_points.cols()),
          result_(result),
          is_opengl(result.camera.intrinsics.convention ==
