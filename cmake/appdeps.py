...
def is_executable_or_shared(file):
    val = subprocess.run(
        args=["file", "-L", "--mime-type", file], capture_output=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-L", action="append", metavar="libdirs", dest="libdirs", type=str)
    parser.add_argument(
        "--outdir", metavar="outdir", dest="outdir", type=str, required=True)
    parser.add_argument(
        "--patchelf-program",
    ...
    copied_deps = copy_deps(deps, outdir, libdirs)

    if args.rpath:
        change_rpath(copied_deps, args.rpath, args.patchelf_program)
